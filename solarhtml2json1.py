...
def parse_table(html_content):
    ...
    for row in rows[0:]:
        ...
        if len(columns) == 2:
            ...
            power_data[omschrijving] = [clean_value(columns[1].text, {'kWh': '', 'W':''})]

    return power_data

def save_power_data(power_data):
    with open('www/power_data1.json', 'w') as outfile:
        json.dump(power_data, outfile, indent=4)

def generate_yaml_from_json():
    with open('www/power_data1.json', 'r') as f:
        power_data = json.load(f)

    inverter_ids = power_data.keys()

    config = 'rest:\n'
    config += '  - resource: http://homeassistant.local:8123/local/power_data1.json\n'
    config += '    sensor:\n'
